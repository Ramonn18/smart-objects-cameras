"""
Head Pose Concatenation Node
=============================
HostNode that combines three separate head pose estimation outputs
(yaw, pitch, roll) into a single NNData tensor for the gaze model.
...
class ConcatenateHeadPose(dai.node.HostNode):
    """Combine yaw, pitch, roll Predictions into a single NNData tensor.

    The head pose estimation model outputs three separate Predictions messages.
    The gaze estimation ADAS model expects them concatenated as a single
    [1, 3] tensor named "head_pose_angles_yaw_pitch_roll".
    """
